...
for filename in filenames:
    ...
    baseconfig['delta_type'] = "adapter"
    dataset_name = filename[len("lora_"):-len(".json")]
    baseconfig["output_dir"] = "/".join(baseconfig["output_dir"].split("/")[:-2]+["adapter",dataset_name])
    json.dump(baseconfig, open(f"adapter_{dataset_name}.json",'w'), indent=4,sort_keys=True)
