...
for filename in filenames:
    ...
    baseconfig['delta_type'] = "compactor"
    baseconfig["delta_lr"] = 3e-3
    dataset_name = filename[len("lora_"):-len(".json")]
    baseconfig["output_dir"] = "/".join(baseconfig["output_dir"].split("/")[:-2]+["compactor",dataset_name])
    json.dump(baseconfig, open(f"compactor_{dataset_name}.json",'w'), indent=4,sort_keys=True)
