...
from examples.delta_args import DeltaArguments
...
def main():
    ...
    delta_model = LoraModel.from_config(delta_args, backbone_model=model)
    delta_model.freeze_module(exclude = delta_args.unfreeze_modules, set_state_dict = True)
    delta_model.save_finetuned(push_to_hub=delta_args.push_to_hub, save_directory=delta_args.delta_saved_name, private=delta_args.push_to_hub_private)
    exit()
    from opendelta.utils.logging import set_verbosity_debug
    set_verbosity_debug()
    delta_model = LoraModel.from_finetuned("ShengdingHu/delta_models", backbone_model=model, use_auth_token=True)
    ...
    if training_args.push_to_hub: # TODO add description here
        delta_model.save_finetuned(push_to_hub=True, save_directory="delta_models", private=True)
        # trainer.push_to_hub(**kwargs)
    else:
        delta_model.save_finetuned(push_to_hub=False, save_directory="delta_models", private=True)
